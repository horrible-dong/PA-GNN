...
def train_one_epoch(graph, model, criterion, data_loader, optimizer, scheduler, device, epoch: int, max_norm: float = 0,
...
@torch.no_grad()
def evaluate(graph, model, data_loader, criterion, device, args, print_freq=10, amp=False):
    model.eval()
    criterion.eval()
